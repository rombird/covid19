...
# 페이지 설정
st.set_page_config(page_title='코로나19 한국 대시보드', page_icon='🚨', layout='wide')
...
if uploaded_confirmed and uploaded_deaths and uploaded_recovered:
    ...
    # diff() --> 차이 (오늘 - 어제)
    # .fillna(0) --> 첫번째 날은 비교대상이 없으니 NaN -> 0으로 채운다
    ...
    tab1, tab2, tab3 = st.tabs(['📈감염 추이', '📊통계 요약', '⚖️비율 분석'])

    # 첫번째 탭 - 감염 추이
    with tab1:
        st.subheader('누적 추이 그래프')
        selected = st.multiselect('표시할 항목을 선택하세요.', ['확진자', '사망자', '회복자'], default=['확진자', '회복자']) # 기본값 -> 확진자, 회복자 그래프 나옴
        ...
        selected_new = st.multiselect('표시할 항목(신규)을 선택하세요.', ['신규 확진자', '신규 사망자', '신규 회복자'], default=['신규 확진자']) # 기본값 -> 신규 확진자
        if selected_new:
    ...
    with tab2:
        ...
        st.dataframe(df_merged.tail(10), use_container_width=True) # 끝에서부터 10개 (날짜가 최신순으로 10개 출력되도록)
    
    # 세번째 탭 - 비율 분석
    with tab3:
        st.subheader('최신일 기준 회복률 / 치명률')
        # 최신날짜가 맨 마지막이니까 -1 --> iloc
        lastest = df_merged.iloc[-1]
        # 확진자 -> confirmed, 사망자 -> deaths, 회복자 -> recovered
        confirmed = lastest['확진자']
        deaths = lastest['사망자']
        recovered = lastest['회복자']
        # 조건연산자 사용 -> 결과를 담는 변수 = 참일 경우 if 조건식 else 거짓일 경우 값
        # 회복률 = (회복자/확진자)*100
...
# 파일이 업로드 되지 않았다면
    ...
    st.info('3개의 CSV파일(확진자, 사망자, 회복자)을 모두 업로드해주세요.')
